...
fig_1
```
Figure one is a graphical representation of the same data. We can see that Colorado, California, and Minnesota have the highest number of breweries, respectively.
...
tail(mbeer, 6)
```
Above are Tables 2 and 3; the first and last 6 rows from the merged data set, respectively.

```{r Problem 3}
```
Dealing with missing (or NA) values:
Analyses dealing solely with ABV will include beers with missing IBU values and vice versa. Analyses dealing with both ABV and IBU will not include any beers that have NA values for either of these variables.
...
```{r Table 4: Mean ABV by State}
head(abv_state, 6)
```
Table 4, above, is the first six rows showing the median ABV by state.
```{r Table 5: Mean IBU by State}
head(ibu_state, 6)
```
Table 5, above, is the first six rows showing the median ABV by state.
```{r Figure 2}
...
fig_8
```
Figure 8, above, is a graph of the average accuracy of 25 values of k looped 100 times using leave-one-out analysis. Based on the results of this graph I chose a k of 9 for my leave-one-out analysis.
...
```{r Table 6}
  CM1
```
Table 6, above, are the results of the train-and-test algorithm. Using this algorithm the computer was able to correctly type beers as either IPA or Ale 81.95% of the time based on ABV and IBU.

```{r Table 7}
  CM2
```
Table 7, above, shows the results of the leave-one-out algorithm. Using this method the computer was able to correctly type beers as either IPA or Ale 84.6% of the time based on ABV and IBU.
...
attach(mbeer)
ABVfit <- aov(ABV ~ State)
TukeyHSD(ABVfit)

ABVfit <- aov(ABV ~ State)
abv_tukey = TukeyHSD(ABVfit)
abv_tukey <- as.data.frame(abv_tukey$State)
...
Table 8, above, shows the states between which there is a statistically significant difference in the mean alcohol content of their beer. Based on the results of this table there could be a potential market for Colorado or Indiana beer in both Massachusetts and Wisconsin.
...
```
Table 9, above, shows the states between which there is a statistically significant difference in the mean IBU of their beer. The resuls of this table show that Wisconsinites prefer beer that is less bitter than that of California, Colorado, Minnesota, or Oregon. If Budweiser were to open a brewery in Wisconsin, I would advise brewing low-IBU beers.
